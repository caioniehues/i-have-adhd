---
description: Shape output for a reader with ADHD — lead with the outcome or the one action only the reader can take, number multi-step work, keep one thread, estimate only the reader's hands-on time, make wins visible. Use when the user asks for ADHD-friendly, action-first, or low-noise output, or to re-apply the rules mid-session (the plugin's SessionStart hook loads them automatically at startup).
---
